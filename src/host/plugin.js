...
export function jarvisCommand(requirement) {
  ...
  if (vague) {
    ...
    // 领域可判（或小需求）→ 走完整人才流程；但**第一步永远是提问，不是建队**（lyj 教训：详细需求也 0 提问直接开干，用户质问"没有疑问直接开干吗"）
    actions = [
      '⓪ **第一步：提问（强制，不许跳过——需求描述详细 ≠ 已吃透）**：CEO+领域专家按各自角色卡专业视角，用 `ask_user_question` 真实问用户（每轮 ≤2 问）——问清"为谁/解决什么/怎样算成功/边界/优先级/约束" + 各角色专业盲点（"从我这行看，这个需求还有哪里不清楚/可能有坑"）；**用户回答前不建队、不派活、不猜着做**；还有疑问就继续问（每轮 ≤2 问），**直到连续两轮用户无新信息（专业角度穷尽）才进下一步**；没问过用户一轮 = 缺陷',
      '① 蒸馏该领域猎头（人才调度层——蒸馏自该领域知名猎头/人才寻访权威，过 distill 双闸）',
      '② 猎头供该领域 CEO（按岗位画像找最合适的带队大佬）→ 贾维斯确认',
      '③ CEO 分析需求 → 向猎头定一个领域专家搭档 → 猎头调度给 CEO 确认',
      '④ CEO+专家 双人打磨需求问用户（各自看法→对齐→问清"为谁/解决什么/怎样算成功"；**若 ⓪ 已问过且无疑问则复用结论，不再重复问**）→ 需求清晰',
      '⑤ 需求无问题 → CEO+专家定团队规模/人员列表 → 猎头逐个调度 → CEO+专家确认够格 → kickoff 开工',
    ]
    guidance = hit.domainDeterminable ? '✅ 领域可从文本判断，启动人才流程（**注意：先提问后建队，提问是第一步**）' : '（小需求/无领域线索但可直接做——按需走或直接做，但仍有疑问就先问）'
  }
  return {
