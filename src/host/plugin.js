...
export function jarvisCommand(requirement) {
  ...
  if (vague) {
    ...
    actions = [
      '⓪ **第一步：按角色卡深度思考需求（强制，不是机械提问）**：CEO+领域专家先按各自角色卡专业视角把需求真正想一遍——产出"我的理解 + 我的真疑问清单"；**有真疑问才用 `ask_user_question` 问用户**（每轮 ≤2 问，只问真拿不准的：为谁/边界/优先级/约束/专业盲点），**思考后确实无疑问（需求详细自洽）就直接进下一步，不许硬凑问题打扰用户**；用户回答前涉及该疑问的功能暂停、不猜着做；**没经过角色卡深度思考就建队 = 缺陷**',
      '① 蒸馏该领域猎头（人才调度层——蒸馏自该领域知名猎头/人才寻访权威，过 distill 双闸）',
      '② 猎头供该领域 CEO（按岗位画像找最合适的带队大佬）→ 贾维斯确认',
      '③ CEO 分析需求 → 向猎头定一个领域专家搭档 → 猎头调度给 CEO 确认',
      '④ **全体开会畅所欲言（头脑风暴）+ 归拢**：每个角色（含 CEO+专家+团队）先各自 ponder 理解需求（有自己的见解）→ 全体开会按各自角色工作方式风格自由发言、互相碰撞、挑战用户信息 → **会议后归拢**成"内部共识 + 分歧点 + 待用户确认清单"（写贾维斯公屏）→ 真疑问才用 `ask_user_question` 问用户（每轮 ≤2 问）；**若 ⓪ 已思考过且无疑问则复用结论，不重复问** → 需求清晰',
