...
export const TOOLS = [
    ...
    handler: async (args) => {
      ...
      if (mode === 'check') {
        ...
        if (hasJarvis || cards || proto || hasProject) {
          ...
          // 续接五件套完整性（代码级归一化：有记忆 ≠ 能续接——续接五件套缺项 = 新会话/追加成员仍会断层，提示补）
          const FIVE = [
            ['当前进度', /当前进度|进行到哪|done|in_progress|pending|任务状态/],
            ['下一步', /下一步|接着干|下一个动作|next/],
            ['未决项', /未决项|未决|open|遗留|遗留问题/],
            ['团队清单', /团队|成员|角色|assignee|清单/],
            ['关键决策与依据', /关键决策|决策.*依据|为什么.*定|否决|选型|拍板/],
          ]
...

  {
    ...
    handler: async (args) => {
      ...
      return {
        ...
        hallucination: `逐句核查幻觉：① source/引用是否真实可查（编造 = 一票否决）；② 数据/数字/案例是否来自真实复现或实证（无出处 = 推测，必须标注）；③ 是否把"我认为"写成"事实"；④ 诚实边界是否声明（信息截止/推测成分）。**防幻觉要"查证痕迹"不要"口头无幻觉"**：对决策里引用的每个关键 source/数据/案例，写清"在哪查的/怎么验证的/查不到则标注'未查证-推测'"——**只写'无幻觉'没给查证痕迹 = 审计走过场 = 不算过**；`,
...

  {
    ...
    handler: async (args) => {
      ...
      const brief = [
        ...
        '',
        '## 二、思考必 ponder（不是只有第一次/只有 CEO 跑——任何"需要思考"的动作都要跑）',
        `- **思考场景清单（必须 ponder 十阶段，固定顺序一个不能少）**：①第一次分析需求 ②会议发言/给观点前（提议/质疑/建议/回应别人）③方案选择/决策 ④难点定位/返工根因 ⑤分歧判断（review 前双方各自 ponder）⑥变更影响评估 ⑦收口判定 ⑧回应/质疑别人的观点——**要产生判断（观点/决策/质疑/结论）→ 先 ponder**；`,
        `- **执行场景可不 ponder**：执行已定方案（按任务描述做，不重新决策）/回报汇报（说做了什么）/查资料读文档（收集信息本身）。`,
        `- **判据一句话**："要不要 ponder" = "这个动作要不要产生一个判断"——要 → 先 ponder，不要 → 直接做。`,
        `- **ponder 十阶段**：interview→shensi→divergence→bagua→plans→converge→score→simulate→debate→synthesis（本单赌注=${stakes}：low 可精简各阶段子 agent 规模，**但阶段不省**）。`,
        ...
        '- **查证通道矩阵**：查资料先真调默认搜索工具（web_search），不可用自动降级——①web_fetch 抓已知 URL ②公开检索端点（平台搜索 API 等）③直接网络抓取；**查到标来源、查不到如实标注"查证受限/基于既有知识推演"，绝不脑补补齐**（编造来源=一票否决）；按你职责查你该查的方向，带事实回会议。',
        '- **需要时找专业同事直连**：跨角色的接口/口径问题直接 agent_teams_send_message 找当事人（不必事事经 captain/CEO 中转）；分歧各自跑 ponder 后交 jarvis_review 裁决。',
        '- **我的模块质量深挖（质量自循环）**：你是自己负责模块的"质量第一责任人"——按本单质量维度清单（`.jarvis/docs/质量维度-<需求>.md`）对自己负责的模块自循环深挖（深挖→修→再深挖直到无问题），**不是等质量评审角色来挑**；连续一轮清单全部维度无毛病才算达标；会议归拢时把碰撞出的新细节方向回报 CEO（回流清单）。',
        '- **不要自己发挥**：成果要对照需求/参照基准做（功能对齐、不加基准里没有的东西）；拿不准先问，不凭感觉加。',
        '- **⚖️ ponder 多次执行是常态（你工作下来会跑很多次 ponder，不是跑一次就完）**：每个"需要产生判断"的动作（会议发言/方案决策/难点定位/回应质疑/变更影响/收口判定…）都先跑 ponder；**高赌注（首次分析需求/方案选择/分歧裁决）满血十阶段+完整子agent；中低赌注（日常判断/发言/回应）精简跑（十阶段骨架一个不少，子agent减少）——阶段不省、必出 run_id；跑前先读历史 run 结论（衔接深化，不从零想）。',
        '- **🔗 上下文准入（追加/新进场成员必做——不读记忆就接手 = 名义追加、实际重建）**：进场后**必须先读项目记忆**（`.jarvis/project.md` 续接五件套[当前进度/下一步/未决项/团队清单/关键决策与依据] + 需求规格 + 方案设计 + 一阶段交付物 + lessons + 黑板决策记录），然后**向 CEO 复述"我理解项目做到哪、为什么这么做、我要接什么"**，CEO 确认理解到位 → 才 claim 任务；**没读记忆/复述不过 → 不许接任务**（先补上下文，否则会重复问已解决的/做出一阶段不一致的东西）。',
        '- **⏱ 卡住限时上报（硬时限，不许自查到地老天荒）**：同一技术问题连续排查约 20-30 分钟仍无进展 → **必须上报**（写黑板：卡在哪 + 已试什么 + 需要什么 + 是否阻塞本任务/其他任务 + 预计何时能给），不许反复自查沉默；**生产/真实操作（升级/部署/改数据）卡住更要立即上报**；同时**执行 CEO 当前最高优先级指令，不被低优先级细节带走**（CEO 说"先X、Y延后"→先做X，Y记下延后）。',
...

  {
    ...
    handler: async (args) => {
      ...
      if (args.groupchat === true) {
        ...
        // 发起模式：建线程 + 输出会议协议（主题/时长/自由发言/主持人盯控/收敛判据）+ 广播话术
        // 真实会议形态（用户定义）：自由发言、随机、可沉默——像真实会议；不是机械排队、不强制每人说
        const protocol = [
          `【群聊会议 · ${topic}】`,
          `主题（要打磨的细节）：${topic}`,
          `时长：最多 ${maxRounds} 轮（超时未收敛 → CEO 判定：继续或归拢）`,
          '发言方式（真实会议形态，用户要求）：',
          '  ① **自由发言**：任何成员有观点/疑问/回应随时说（不排队、不轮流、不强制）——像真实会议，谁想到谁说；',
          '  ② **可沉默**：没观点/没疑问的成员可以不发言（真实会议也有人不说话）——沉默≠缺席，议题相关才需要发声；',
          '  ③ **接话碰撞**：看到别人观点，愿意就接（同意/反驳/补充+理由）——碰撞靠自愿接话，不靠轮流；',
          '  ④ 所有人能看到所有人的发言（共享线程=群聊，read 线程即见全员）。',
          '主持人（CEO）盯控（真实会议主持人的职责）：',
          '  - **该发声的没发声**：某个议题明显与该角色相关（测试对验收、实现对方案）而 TA 没说话 → CEO 点名问"你怎么看"（像真实会议主持人叫到人），不是强制，是确保相关视角被听到；',
          '  - **观点没人回应**：某观点提出后无人接话 → CEO 追问"有人同意/反对吗"（防观点悬空）；',
          '  - **沉默允许**：点完名仍没话说 = 真没观点，尊重沉默。',
          `收敛判据（用户要求）：所有细节都被确认没问题（每个议题/细节有成员明确"无问题/已闭环"）才散会；有遗留细节 → 继续下一轮。`,
        ...
        const broadcast = `【群聊会议发起 · ${topic}】请到共享线程发言（真实会议形态：自由发言、可沉默、接话碰撞，CEO 主持人盯控该发声的没发声会点名；最多 ${maxRounds} 轮，所有细节没问题才散会）：${threadRel}`
        return {
          verdict: `群聊会议已发起：主题=${topic}，时长≤${maxRounds}轮，自由发言（可沉默）+主持人(CEO)盯控点名，收敛=所有细节无问题才结束`,
...

  {
    name: 'jarvis_clarify',
    description:
      '需求澄清引导器（REFORM-CLARIFY 机制实现，领域无关）：用户需求模糊时，CEO 以专业角度分析用户需求并引导用户回答问题（不是干等/代为澄清）。模式：①analyze=CEO 专业分析先行——把模糊需求按行业常识/同类需求模式/可行域翻译成 5 角度候选问题（P1 场景/P2 现状/P3 痛点/P4 期望/P5 验收），返回候选问题清单+模糊度判定；②trigger=蒸馏触发判定（机械可判 T1-T5：T1 5 角度中≥2 个候选问题为空或仅含无具体名词追问（"再说说/展开讲讲"不计，须含对象/场景/行业词才算数）、T2 无同类卡与原型、T3 ≥2 角度命中行业术语但引不出行业依据（规范名/source/同类案例出处）、T4 用户回答超 CEO 专业范围、T5 单人 3 轮未收敛——任一触发=建议现场蒸馏该需求领域真实大佬（软件→软件大佬、制造→制造大佬，领域由需求决定插件无预设），六段式卡过 jarvis_distill 校验后双人进场）；③ask=三阶提问（开放→聚焦→确认，**问尽所有真疑问、不设数量上限**——收敛判据是"无剩余真疑问"不是"固定几个"）生成引导话术；④duo=双人协作方案 A 判据（同刻一人主问：用户回答前主问者 from≤1 可数，补充消息须标注"补充"；CEO 域=P5 验收+与交付相关的 P2 子集[频率/负责人/时限]，大佬域=P1-P4 场景/细节/专业盲区，防重复=问题含场景/现状/痛点/期望/验收关键词即同域）；⑤confirm=澄清完成判定（需求本质重述三段式[为谁/解决什么/怎样算成功可判定]+假设分级[已确认/推断待确认]，经用户确认=完成，缺一禁止进拆解）。铁律：推断不得冒充用户已确认；用户拒绝/失联=连续 2 轮无有效回答→按 CEO 推断+显式标注继续（3 轮=挂起）；澄清产出过 B16 黑板翻译/B22 可行性闸/B15 分层（标注为黑板决议目标态）；蒸馏查证优先用默认搜索工具 web_search（先真调再判断），真失败才降级（curl 验证 source+标注推演，降级须用户知情同意）。',
    parameters: {
      type: 'object',
      properties: {
    ...
    handler: async (args) => {
      ...
      if (mode === 'analyze') {
        ...
        return {
          ...
          vague,
          verdict: vague
            ? `需求模糊 → 按 5 角度候选问题引导用户回答（**问尽真疑问、不设数量上限**，先开放后聚焦，最后确认式）；用户回答后跑 trigger 判定是否需蒸馏行业大佬`
      ...
      if (mode === 'trigger') {
        ...
        return {
          ...
          verdict: triggered ? `触发蒸馏 → 现场 web 蒸馏「${industry || '该需求领域'}」真实权威（优先用默认搜索工具 web_search 查证，真失败才降级：curl 验证 source+标注推演，降级须用户知情同意）→ jarvis_distill 校验 → 双人协作` : `不触发 → CEO 单人三阶引导（开放→聚焦→确认）`,
        }
      }
      if (mode === 'ask') {
        // ── 三阶提问话术（开放→聚焦→确认，问尽真疑问、不设数量上限）──
        ...
        questions = questions.slice(0, 12) // 一次问尽真疑问（不设 2 个上限——用户要求"目的是对其需求所有疑问"，收敛判据=无剩余真疑问）
        return {
          mode: 'ask',
          questions,
...
export function jarvisCommand(requirement) {
  ...
  if (vague) {
    ...
    actions = [
      '⓪ **先提问（不是干等）**：用 `ask_user_question` 请用户说具体想干什么（想做什么/解决什么问题/给谁用——不用问"哪个领域"这种抽象问题），从回答判断领域方向；再按角色卡专业视角追问细节直到无疑问',
      '① 回复用户：「你想做什么/解决什么问题/给谁用？能具体说说吗」——请用户说详细点',
      '② 从用户的具体描述判断领域方向（如"给员工做考勤"→软件/HR；"养鸡场自动喂食"→农业/自动化）——循环请补充说到能判出',
      '③ 领域判出后 → 蒸馏该领域猎头（人才调度层）→ 猎头供该领域 CEO → CEO 分析需求定一个领域专家 → 双人打磨需求（继续问直到无疑问）',
    ]
    guidance = '⚠️ 判不出领域方向（需求太模糊/无领域线索）——先用 ask_user_question 请用户说具体想干什么（说详细点），从中判断领域，再启动人才'
  } else {
    // 领域可判（或小需求）→ 走完整人才流程；但**第一步永远是按角色卡深度思考需求，真疑问才问，不是机械提问**（lyj 教训：详细需求 0 思考直接开干；用户要求"要的是真正的思考，不是必须问问题——需求很详细也可能没问题"）
    actions = [
      '⓪ **第一步：按角色卡深度思考需求（强制，不是机械提问）**：CEO+领域专家先按各自角色卡专业视角把需求真正想一遍——产出"我的理解 + 我的真疑问清单"；**有真疑问才用 `ask_user_question` 问用户**（问尽所有真疑问、不设数量上限，只问真拿不准的：为谁/边界/优先级/约束/专业盲点），**思考后确实无疑问（需求详细自洽）就直接进下一步，不许硬凑问题打扰用户**；用户回答前涉及该疑问的功能暂停、不猜着做；**没经过角色卡深度思考就建队 = 缺陷**',
