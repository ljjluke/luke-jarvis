...
> **实施状态注（2026-09-03）**：I10（client 注册 UI slot 只读展示 .jarvis/board.json）已按方案实施（0.2.2 黑板按钮+浮层），经使用验证为低价值（只读弹窗与对话流重复），已在 0.2.3 **整文件移除 client 半边**。host 侧 board 数据/API（`.jarvis/board.json` + `/api/luke-jarvis/board`）保留；本方案的专章 D（client UI 呈现思路）留作 3D 办公室阶段重新设计的参考。
