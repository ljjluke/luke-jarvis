...
**先查共享信息再问（能答先答，答不了才问——不把已有答案当问题抛给用户/客户）**：五诊画像（天/地/人/法/物）开始前，**先查当前上下文里已有的产品信息**：会议讨论结论/黑板（.jarvis/board.json）/需求规格（.jarvis/docs/需求规格-*.md）/方案设计/项目沉淀（project.md/lessons.md）——**每个维度先问"这个维度在共享信息里有没有答案？"有 → 直接采入画像（标注来源），不重复问用户**。全部维度查完后，**只剩真正缺失的（需求不足/信息残缺/共享信息没有的）才 AskUserQuestion 问用户/客户**（如"客户要什么/怎样算成功"共享里没写 → 才问）。⛔ 禁止"共享信息里已有答案还问用户"（浪费客户时间=没看会议）也禁止"需求真缺却不问硬猜"（缺=问，不许脑补）。

**🔗 衔接历史思考（修复"每次 ponder 全新流程"缺陷——不读之前结论=重复想+可能倒退）：interview 前先读本项目历史 ponder 结论**（`.jarvis/ponder-runs/<run_id>/outcome.json` 或 synthesis/end-state.md，按问题相关性找最近的 1-3 个）——**已有结论直接采入画像（标注来源=上次 run_id+结论），本次是"确认/深化/推翻"上次结论，不是从零想**；上次的 confirmed/refuted/corrected 结果必须纳入（用户确认过的=基准，反驳过的=避坑）；**没有历史 run → 从零想**（正常，首次）。
...
| 🔍 八卦镜 | `stages/bagua.json` | 发现盲点 | **必须等发散产出后**再起子 agent（吃发散共识）；每维度一个 agent（共8个并行），**必须等全部8个维度 agent 返回并展示盲点表格后**，主线程再汇总为 key_finding 交给方案。不允许在部分维度还在排查时就跳到方案步骤。**⛔ 产出必须覆盖全部 8 个维度（F1驱动力/F2基础/F3变化/F4渗透/F5风险/F6依附/F7边界/F8平衡），每个维度有实质盲点内容——有多少盲点就对应解决多少，一个都不能少、不许挑部分维度总结、不许只列维度名没内容、不许另起炉灶写自己的八项**（orchestrate step 会硬校验：缺维度/内容空/自定义维度 → BLOCKED 打回补齐后再存） |
