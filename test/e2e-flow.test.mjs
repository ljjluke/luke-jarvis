...
// ── 2b. 空洞但关键词齐全的"浅层卡"必须被深度闸拦下 ─────────
const SHALLOW = [
  '身份定位：企业高管。',
  '思维模型：很强（跨域复现：有；生成力：有）。',
  '核心方法论：方法论。',
  '代表作品：作品——只借鉴框架，不冒充署名。',
  '决策红线：底线。',
  '语言风格：简洁。',
  '协同架构：位置=并行；依赖=产品契约；介入时机=现在；协同方式=实时讨论（并行非串行）。',
...
// ── 3. jarvis_distill 校验（合格深度卡应过；浅层卡应被拦） ──
step('3. jarvis_distill 证据链+深度双硬闸（合格深度卡过 / 空洞浅层卡拦）', async () => {
  const missing = validateCardShape(GOOD_CEO, true)
  const r = await jd('jarvis_distill').handler({ role: 'CEO', card: GOOD_CEO, isCeo: true })
  console.log(`  深度卡: ok=${r.ok} depth=${r.depthScore}`)
  allOk = ok('合格深度 CEO 卡通过（缺项为空 + 深度达标）', r.ok === true && missing.length === 0 && r.depthScore >= 60) && allOk
  // 空洞但关键词齐全 → 深度闸必须拦（防"标题齐全内容空洞"的浅层蒸馏）
  const rs = await jd('jarvis_distill').handler({ role: 'CEO', card: SHALLOW, isCeo: true })
  console.log(`  浅层卡: ok=${rs.ok} depth=${rs.depthScore}`)
  allOk = ok('浅层卡被深度闸拦下（结构齐全≠有深度）', rs.ok === false && (rs.depthScore ?? 100) < 60) && allOk
})
...
const roleCard = [
  ...
  '保真度：一手0.6；矛盾保留。',
  '诚实边界：信息截止；推测已标。',
  'source：https://www.riskjournal.org/articles/risk-interview（本次 web 检索确认该访谈原文）',
  '防冒名声明：只借鉴框架，不冒充署名。',
].join('\n')
...
step('汇总', () => {
  ...
  console.log('流程完整性：19 个工具 + /jarvis 命令，各环节均有硬闸，链路闭环。')
